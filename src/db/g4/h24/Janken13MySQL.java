 ...
 * 8.  勝率をSQLで計算して表示
 * 
  create view  jankenLog02_v as select j.no ,j.you,u.label as YouLabel,j.pc,p.label as PCLabel,j.result,r.label  as ResultLabel from jankenLog02 j
...
public class Janken13MySQL {
	...
	public static void showNumOfHands() throws SQLException{
		Statement stmt = (Statement) conn.createStatement();
		String sql = "SELECT COUNT( * ) as count ,you, YouLabel  FROM jankenLog02_v GROUP BY you Order by you";
		ResultSet rs = stmt.executeQuery(sql);
	...
	public static void showStatistics() throws SQLException{
		Statement stmt = (Statement) conn.createStatement();
		String sql = "SELECT COUNT( * ) as count ,result  FROM jankenLog02 GROUP BY result";
		ResultSet rs = stmt.executeQuery(sql);
